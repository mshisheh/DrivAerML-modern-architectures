"""
Data Loader for GraphCast on DrivAerNet

Prepares surface mesh data for GraphCast training.
...
class GraphCastDataset(Dataset):
    """
    Dataset for GraphCast surface pressure prediction.
    ...
    def __init__(
        ...
        if self.verbose:
            print(f"GraphCastDataset initialized with {len(self.design_ids)} designs")
...
def load_run_ids(split_dir: str) -> Tuple[List[int], List[int], List[int]]:
    ...
    Returns:
        train_ids, val_ids, test_ids as lists of integers
    """
